const Info = {
  intro: {
    ...
    text: `CheeseJS is a simple yet functional Web Component library for building customizable Chess UI. It is built with Typescript using the
    StencilJS library (not a frameowork!) and supports move validations according to the rules of Chess. This is the website for the library
    showcasing its features, addressing its limitations, sharing my future plans with the library and writing about the hows
    and the whys I decided to build it. As of now, the library only has 3 Web Components, that get incrementally
  ...
  checkerBoard: {
    ...
    to get started with the project and see if I could build a simple checkered board or not. I also had to make sure to
  ...
  chessBoard: {
    ...
    provided then it starts with the default starting position. The pieces can then be moved on the board however for this
    ...
    create an HTML string from an FEN, making sure the pieces could be moved (first drag and drop was implemented then I later
    ...
    and not game states like whoss move it is, if a color has castling rights etc because this component does not actually create a playable
  ...
  analysisBoard: {
    ...
    and standard game rules to it. Not only that, <analysis-board> also remembers various game states such as whose move it is and takes into account special
    ...
    playable componenet. The only things missing from making it an actual game are the clocks and a mate detection system, both of which are part
    of my future plans for this library`,
  },
  process: {
    heading: `Why (and a bit of how) I built this`,
    text: [
      `As a long time Chess enjoyer, I always wanted to build some cool Chess programs. I had built a website for r/AnarchyChess
    ...
    no experience in ML and AI. Then the idea of the Web Component library came.`,
      ...
      `Web Components are natively built into the browser and can be shared and used in other programs as easily as using a regular HTML element. In this
    ...
    and build the whole thing from scratch without any external guidance or help, and I would say I have been largely successful in that regard.`,
    ...
    made the whole drag and drop and click events possible`,
    ...
    For example, in the starting position, boardMap['a']['1'] would return 'R', a white rook which is on the 'a1' square`,

      `Now let me tell you about one silly bug that ruined my life for like 4 days. Objects are passed as references in JavaScript instead of values, I of course knew that and I also
    ...
    updates the board, there would be random pieces on random squares on the board, like a pawn on c4 or an extra black king on e6. I could not for the life of me figure out what was going wrong
    and it was driving me insane. So after about 4 days of pulling my hair out, I finally came to the solution. A simple JSON.parse(JSON.stringify(boardMap)) solved the bug as it deep clones
    ...
    creating a Chess engine of my own and I plan to do it in either Go or Rust. I strongly believe Chess programming has something for everyone no matter what their domain is`,
    ],
  },
  limitations: {
    heading: `Drawbacks of the library`,
    text: [
      `There are two main drawbacks of this project that I would like to address.`,

      `1. There is a square background behind a piece when dragging it. It makes it look really ugly and I cannot get rid of it. I have tried a few solutions like
    ...
    of the background color when dragging. Please consider contacting me or contributing if you know how to fix it`,
  ...
  plans: {
    ...
    text: [
      `I have a few plans ready for the future regarding this project, They are:
    ...
    I need to create a customizable Chess clock and a sofisticated solution for detecting checkmates and stalemates`,

      `2. Once that is done, I can start building a new component with custom fairy pieces and rules Since I already
    have a system ready for validations and game rules, it should not be that difficult especially for pieces that combine other pieces (like the Knook,
